...
//asigna del almacenamiento local las variables a nuevas variables

function send() {
	get_word = document.getElementById("word").value;
	word = get_word.toLowerCase();
	console.log("word in lowerCase = " + word);
//creara una nueva variable llamada get_word y le dara la capacidad de cambiar las letras mayusculas a minusculas

    charAt1 = word.charAt(1);
	console.log(charAt1);
// localizara la posicion de la primera letra

	lenght_divide_2 = Math.floor(word.length/2);
	charAt2 = word.charAt(lenght_divide_2);
	console.log(charAt2);
//dividira la palabara a la mitad y si el numero termina en decimal regresara a la letra anterior

    lenght_minus_1 = word.length - 1; 
    charAt3 = word.charAt(lenght_minus_1); 
	console.log(charAt3);
//regresa la posicion del caracter a longitud -1

    remove_charAt1 = word.replace(charAt1, "_");
    remove_charAt2 = remove_charAt1.replace(charAt2, "_");
    remove_charAt3 = remove_charAt2.replace(charAt3, "_");
	console.log(remove_charAt3);
//reemplaza todas las variables por "_"
    ...
    //el boton "check_button" ser color azul
    row =  question_word + input_box + check_button ; 
    document.getElementById("output").innerHTML = row;
document.getElementById("word").value = "";
}
//detectara si la respuesta es correcta si no seguira siendo turno del jugador correspondiente
